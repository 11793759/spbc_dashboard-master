...
if(array_key_exists("SHOW_WARNINGS", $_GET)) {
...
/* print "The GNA Dashboard is down due to some technical difficulties. Will be back up ww4.3. Thanks, Brian"; */
/* exit(); */
...
$local_run = FALSE;
if($_SERVER["HTTP_HOST"] == "SPBC_DASHBOARD") {
    $local_run = TRUE;
} else {
...
// ZQA
$values["AREA"] = sprintf("%0d", get($handle, "${config}-area"));
$values["POWER"] = sprintf("%0d", get($handle, "${config}-power"));
$values["FPGA_SCORE"] = sprintf("%0d", get($handle, "${config}-fpga_score"));
